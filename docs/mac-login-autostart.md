...
## 推奨構成
- ログイン項目には [`start_app.command`](/Users/hasegawaakihiko/Codex/New%20project/start_app.command) を追加する
- これは `localhost:8000` のローカルサーバーだけを起動する軽い版
- `search-index` やサムネイルを更新したい時だけ [`start_app_with_build.command`](/Users/hasegawaakihiko/Codex/New%20project/start_app_with_build.command) を手動で使う
...
3. `ログイン項目と機能拡張`
4. `ログイン時に開く項目` の `+`
5. このプロジェクトの [`start_app.command`](/Users/hasegawaakihiko/Codex/New%20project/start_app.command) を選ぶ
