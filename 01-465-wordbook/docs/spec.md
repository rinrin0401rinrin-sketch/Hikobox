...
- 20名セットの追加データも、この1名の表記・命名・出典記録を踏襲している
...
- 検証スクリプトによる基本整合性チェック
- 20名セットの JSON、写真、レビュー台帳
- batch-02-50 の `hr-0060` までの source 確認、`career` 反映、再開用台帳
...
## 現時点の判断
- 完成見本1名は差し替え段階を終えた
- 検証用20名セットは投入済みで、初期表示、前後移動、表裏切替、絞り込み、狭幅確認まで完了
- batch-02-50 は `hr-0060` まで source 確認と `career` 反映を完了し、`index.json`、`manifest.json`、`worklist.tsv`、`review.tsv` を同期済み
